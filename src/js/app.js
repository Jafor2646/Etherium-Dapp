App = {
  ...
  doctorSignin: function () {
    App.contracts.PatientManagement.deployed()
      .then(function (instance) {
        return Promise.all([instance.admin(), instance.doctors(App.account)]);
      })
      .then(function (results) {
        // var adminAddress = results[0];
        let adminAddress = results[0];
        var isDoctor = results[1];
        console.log(App.account, adminAddress);
        if (
          isDoctor &&
          adminAddress.toLowerCase() !== App.account.toLowerCase()
        ) {
          console.log("Doctor signing in. Initiating transaction...");
          return window.ethereum.request({
            method: "eth_sendTransaction",
            params: [
              {
                from: App.account,
                to: adminAddress, // Send transaction to admin's address
                gas: "50000",
                value: "0", // No Ether is transferred, can adjust as needed
          ...
          if (adminAddress.toLowerCase() === App.account.toLowerCase()) {
            alert("Admin cannot sign in as a doctor");
          } else {
            alert("You are not registered as a doctor.");
          }
          throw new Error("Sign-in attempt failed"); // Stop further execution
        }
      })
      .then(function (txHash) {
        console.log("Transaction Hash:", txHash);
        window.location.href = "doctorDashboard.html"; // Redirect to the doctor dashboard
      })
      .catch(function (error) {
  ...
  AllPatients: async function () {
    try {
      ...
      const vaccine = ["NotVaccinated", "OneDose", "TwoDose"];
  ...
  UpdatePatient: async function (
    ...
    updatePatientForm.onsubmit = async function (event) {
      ...
      const updatedVaccineStatus = document.getElementById(
        "updateVaccineStatus"
      ).value;
  ...
  showHighestDistrictPatients: function () {
    App.contracts.PatientManagement.deployed()
      ...
      .then(function (result) {
        const districtWithHighestDiv = document.getElementById(
          "districtWithHighest"
        );
        districtWithHighestDiv.innerHTML =
          "<b>District with Highest Patients: </b>" + result;
      })
      .catch(function (error) {
        console.error(error);
      });
  ...
  listenForEvents: function () {
    App.contracts.PatientManagement.deployed()
      .then(function (instance) {
        ...
        instance
          .DistrictWithMostPatientsChanged(
            {},
            {
              fromBlock: 0,
              toBlock: "latest",
